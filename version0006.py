...
def get_caves_adjacent_to(cave_number):
  ...
  elif cave_number == 19:  # change this line
    return [cave_number - 1, cave_number - 3]
...
def play():
    ...
    visited_caves = []  # change this line
    while player_location == wumpus_location:
        player_location = random.choice(cave_numbers)

    while True:
        print("You are in cave {}.".format(player_location))
        draw_cave(cave_numbers, wumpus_location, player_location, visited_caves)
        print()
        print("From here, you can see caves: {}".format(", ".join(str(cave) for cave in get_caves_adjacent_to(player_location))))
        print()
        for hint in get_hints(cave_numbers, wumpus_location, player_location):
            print(hint)

        player_choice = ask_for_cave(cave_numbers, player_location)
        visited_caves.append(player_choice)
        if player_choice == wumpus_location:
            print("You have been eaten by the Wumpus!")
            break
        player_location = player_choice
